bio_results <- function (values, data, res, con) {
  library(broom)
  results <- tibble::tibble(Ind = c("Shannon", "Riqueza", 
                                    "LT50Obj1", "Densidad", "DensidadObj1", "NivelTrofico", 
                                    "Biomasa", "BiomasaObj1", "LT50Obj2", "LT50Obj3", "LT50Obj4", 
                                    "LT50Obj5", "LT50Obj6", "LT50Obj7", "LT50Obj8", "BiomasaObj2", 
                                    "BiomasaObj3", "BiomasaObj4", "BiomasaObj5", "BiomasaObj6", 
                                    "BiomasaObj7", "BiomasaObj8", "DensidadObj2", "DensidadObj3", 
                                    "DensidadObj4", "DensidadObj5", "DensidadObj6", "DensidadObj7", 
                                    "DensidadObj8"), e = NA, p = NA, string = NA, color = NA, 
                            model = list(NA), plot = list(NA))
  if ("Indice de diversidad de Shannon" %in% values$indB) {
    model <- shannon(data, values$comunidad) %>% turfeffect(res, 
                                                            con, type = "bio", year.imp = values$ano.imp)
    TidyModel <- model$TidyModel %>% filter(term == "ZonaReserva:Post1")
    model <- model$model
  ...
  if ("Riqueza" %in% values$indB) {
    model <- richness(data, values$comunidad) %>% turfeffect(res, 
                                                             con, type = "bio", year.imp = values$ano.imp)
    TidyModel <- model$TidyModel %>% filter(term == "ZonaReserva:Post1")
    model <- model$model
  ...
  if ("Organismos > LT_50" %in% values$indB & length(values$objsp$sp) > 
      0) {
    model <- fish_size(data = data, location = values$comunidad, 
                       species = values$objsp$sp[1]) %>% turfeffect(reserve = res, 
                                                                    control = con, type = "bio", year.imp = values$ano.imp)
    TidyModel <- model$TidyModel %>% filter(term == "ZonaReserva:Post1")
    model <- model$model
    ...
    results$plot[[3]] <- mpa_plot4(model, y.lab = paste("Densidad relativa", 
                                                        values$objsp$sp[1], "\n(Org > LT_50/TotOrg Transecto)"))
  }
  if ("Densidad" %in% values$indB) {
    model <- density(data, values$comunidad) %>% turfeffect(res, 
                                                            con, type = "bio", year.imp = values$ano.imp)
    TidyModel <- model$TidyModel %>% filter(term == "ZonaReserva:Post1")
    model <- model$model
  ...
  if ("Densidad de especies objetivo" %in% values$indB & length(values$objsp$sp) > 
      0) {
    model <- density(data = data, location = values$comunidad, 
                     species = values$objsp$sp[1]) %>% turfeffect(res, 
                                                                  con, type = "bio", year.imp = values$ano.imp)
    TidyModel <- model$TidyModel %>% filter(term == "ZonaReserva:Post1")
    model <- model$model
    ...
    results$plot[[5]] <- mpa_plot4(model, y.lab = paste("Densidad", 
                                                        values$objsp$sp[1], "\n(Organismos/Transecto)"))
  }
  if ("Nivel trofico" %in% values$indB) {
    model <- trophic(data, values$comunidad) %>% turfeffect(res, 
                                                            con, type = "bio", year.imp = values$ano.imp)
    TidyModel <- model$TidyModel %>% filter(term == "ZonaReserva:Post1")
    model <- model$model
  ...
  if ("Biomasa" %in% values$indB) {
    model <- fish_biomass(data, values$comunidad) %>% turfeffect(res, 
                                                                 con, type = "bio", year.imp = values$ano.imp)
    TidyModel <- model$TidyModel %>% filter(term == "ZonaReserva:Post1")
    model <- model$model
  ...
  if ("Biomasa de especies objetivo" %in% values$indB & length(values$objsp$sp) > 
      0) {
    model <- fish_biomass(data = data, location = values$comunidad, 
                          species = values$objsp$sp[1]) %>% turfeffect(res, 
                                                                       con, type = "bio", year.imp = values$ano.imp)
    TidyModel <- model$TidyModel %>% filter(term == "ZonaReserva:Post1")
    model <- model$model
    ...
    results$plot[[8]] <- mpa_plot4(model, y.lab = paste("Biomasa", 
                                                        values$objsp$sp[1], "\n(Kg/Transecto)"))
  }
  if ("Organismos > LT_50" %in% values$indB & length(values$objsp$sp) > 
      1) {
    ...
    results$plot[[n_obj]] <- mpa_plot4(model, y.lab = paste("Densidad relativa", 
                                                            values$objsp$sp[2], "\n(Org > LT_50/TotOrg Transecto)"))
  }
  if ("Organismos > LT_50" %in% values$indB & length(values$objsp$sp) > 
      2) {
  ...
  if ("Biomasa de especies objetivo" %in% values$indB & length(values$objsp$sp) > 
      1) {
    model <- fish_biomass(data = data, location = values$comunidad, 
                          species = values$objsp$sp[2]) %>% turfeffect(res, 
                                                                       con, type = "bio", year.imp = values$ano.imp)
    TidyModel <- model$TidyModel %>% filter(term == "ZonaReserva:Post1")
    model <- model$model
    n_obj <- 16
    ...
    results$color[n_obj] <- bio_score(TidyModel)
    results$model[[n_obj]] <- model
    results$plot[[n_obj]] <- mpa_plot4(model, y.lab = paste("Biomasa", 
                                                            values$objsp$sp[2], "\n(Kg/Transecto)"))
  }
  if ("Biomasa de especies objetivo" %in% values$indB & length(values$objsp$sp) > 
      2) {
    model <- fish_biomass(data = data, location = values$comunidad, 
                          species = values$objsp$sp[3]) %>% turfeffect(res, 
                                                                       con, type = "bio", year.imp = values$ano.imp)
    TidyModel <- model$TidyModel %>% filter(term == "ZonaReserva:Post1")
    model <- model$model
    n_obj <- 17
  ...
  if ("Biomasa de especies objetivo" %in% values$indB & length(values$objsp$sp) > 
      3) {
    model <- fish_biomass(data = data, location = values$comunidad, 
                          species = values$objsp$sp[4]) %>% turfeffect(res, 
                                                                       con, type = "bio", year.imp = values$ano.imp)
    TidyModel <- model$TidyModel %>% filter(term == "ZonaReserva:Post1")
    model <- model$model
    n_obj <- 18
  ...
  if ("Biomasa de especies objetivo" %in% values$indB & length(values$objsp$sp) > 
      4) {
    model <- fish_biomass(data = data, location = values$comunidad, 
                          species = values$objsp$sp[5]) %>% turfeffect(res, 
                                                                       con, type = "bio", year.imp = values$ano.imp)
    TidyModel <- model$TidyModel %>% filter(term == "ZonaReserva:Post1")
    model <- model$model
    n_obj <- 19
  ...
  if ("Biomasa de especies objetivo" %in% values$indB & length(values$objsp$sp) > 
      5) {
    model <- fish_biomass(data = data, location = values$comunidad, 
                          species = values$objsp$sp[6]) %>% turfeffect(res, 
                                                                       con, type = "bio", year.imp = values$ano.imp)
    TidyModel <- model$TidyModel %>% filter(term == "ZonaReserva:Post1")
    model <- model$model
    n_obj <- 20
  ...
  if ("Biomasa de especies objetivo" %in% values$indB & length(values$objsp$sp) > 
      6) {
    model <- fish_biomass(data = data, location = values$comunidad, 
                          species = values$objsp$sp[7]) %>% turfeffect(res, 
                                                                       con, type = "bio", year.imp = values$ano.imp)
    TidyModel <- model$TidyModel %>% filter(term == "ZonaReserva:Post1")
    model <- model$model
    n_obj <- 21
  ...
  if ("Biomasa de especies objetivo" %in% values$indB & length(values$objsp$sp) > 
      7) {
    model <- fish_biomass(data = data, location = values$comunidad, 
                          species = values$objsp$sp[8]) %>% turfeffect(res, 
                                                                       con, type = "bio", year.imp = values$ano.imp)
    TidyModel <- model$TidyModel %>% filter(term == "ZonaReserva:Post1")
    model <- model$model
    n_obj <- 22
  ...
  if ("Densidad de especies objetivo" %in% values$indB & length(values$objsp$sp) > 
    ...
    results$color[n_obj] <- bio_score(TidyModel)
    results$model[[n_obj]] <- model
    results$plot[[n_obj]] <- mpa_plot4(model, y.lab = paste("Densidad", 
                                                            values$objsp$sp[2], "\n(Organismos/Transecto)"))
  }
  if ("Densidad de especies objetivo" %in% values$indB & length(values$objsp$sp) > 
      2) {
